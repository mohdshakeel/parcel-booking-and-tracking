...
import { SlidersHorizontal,Search, X,Truck} from "lucide-react";
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <div className="flex flex-col gap-4 border-b border-gray-200 px-4 py-4 sm:px-5 lg:flex-row lg:items-center lg:justify-between dark:border-gray-800">
        ...
        <div className="flex flex-col gap-4 sm:flex-row lg:items-center">
          {/* Status Filters */}
          {["All", "In-Transit", "Idle", "Processing"].map(label => (
  <button
    key={label}
    onClick={() => {
      setStatus(label);
          ...
          <table className="min-w-full w-full table-auto">
            <thead>
              <tr className="border-b border-gray-200 dark:divide-gray-800 dark:border-gray-800">
                ...
                {/* Other table heads */}
                {["Vehicle Type", "Vehicle Number", "Vehicle Capacity & Volume", "Vehicle Modal", "Registration", "Current Location & Status"].map(
                  (head, i) => (
                    <th
                      key={i}
